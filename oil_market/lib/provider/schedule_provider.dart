...
class ScheduleProvider extends ChangeNotifier {
  ...
  Map<DateTime, List<SchedulesModel>> cache = {};  // ➌ 일정 정보를 저장해둘 변수
  ...
  void createSchedule({
    ...
    try {
      final savedSchedule = await repository.createSchedules(schedule: schedule);  // API 요청을 합니다.
  ...
  void deleteSchedule({
    ...
    try {
      await repository.deleteSchedules(id: id); // ➊ 삭제함수 실행
    } catch (e) {
      cache.update(
        // ➋ 삭제 실패시 캐시 롤백하기
        date,
